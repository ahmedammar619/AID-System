...
if($for == 'confirmAll' || $for == 'deselectAll' || $for == 'selectAll'){
    //insert delivery log for each recipient
    $recipients = $_GET['recipients'];
    $recipients = explode(',', $recipients);
    $recipients = array_map('intval', $recipients);
    foreach($recipients as $recipient){
        $sql = "insert into delivery_logs (volunteer_id, recipient_id, status) values ($volunteerID, $recipient, '$forEdited')";
        $conn->query($sql);
    }
}
else if($for == 'pickup'){
    $sql = "insert into delivery_logs (volunteer_id, recipient_id, status) values ($volunteerID, NULL, '$forEdited')";
    $conn->query($sql);
}
else{
    $sql = "insert into delivery_logs (volunteer_id, recipient_id, status) values ($volunteerID, $recipient_id, '$forEdited')";
    $conn->query($sql);
}
...
if(isset($_SESSION['volunteerID'])){
    ...
    //     $sql = "Update delivery set selected_date = NOW(), volunteer_id = $volunteerID where (recipient_id in ($recipients)) and ((selected_date IS NULL OR selected_date < NOW() - interval 10 minute) or volunteer_id is null) and (status = 'Pending' or status= 'Failed')";
    //     $conn->query($sql);
    // }
    else if($for == 'confirm'){
